...
class SynthesisCreate extends Controller {
    ...
    public function index() {
        ...
        /* Formats */
        $ai_formats = require APP_PATH . 'includes/aix/ai_syntheses_' . $this->user->plan_settings->syntheses_api . '_formats.php';
        ...
        $view = new \Altum\View(THEME_PATH . 'views/synthesis-create/index', (array) $this, true);
        ...
        switch($this->user->plan_settings->syntheses_api) {
            ...
            case 'aws_polly':
                ...
                $client = new \Aws\Polly\PollyClient([
                    'version' => 'latest',
                    'region' => $this->user->plan_settings->exclusive_personal_api_keys ? $this->user->preferences->aws_region : settings()->aix->region,
                    'credentials' => [
                        'key' => $this->user->plan_settings->exclusive_personal_api_keys ? $this->user->preferences->aws_access_key : settings()->aix->access_key,
                        'secret' => $this->user->plan_settings->exclusive_personal_api_keys ? $this->user->preferences->aws_secret_access_key : settings()->aix->secret_access_key,
                    ],
                ]);
