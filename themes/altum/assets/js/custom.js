...
document.querySelectorAll('[type="submit"][name="submit"]:not([data-is-ajax])').forEach(element => {
    element.addEventListener('click', event => {
        ...
        /* Normal loading ajax or with overlay */
        let type = 'normal';

        if(element.getAttribute('data-is-ajax-full')) {
            type = 'full';
        }
        ...
        if(type == 'full') {
            /* Disable the form as well */
            element.form.classList.add('disabled');
            element.form.classList.add('container-disabled-simple');
        }
...
const pause_form = element => {
    ...
    /* Disable the button */
    let submit_button = element.querySelector('button[type="submit"]');
    submit_button.setAttribute('disabled', 'disabled');

    /* Save the current button text */
    submit_button.setAttribute('data-inner-text', submit_button.innerText);

    /* Show a loading spinner instead of the text */
    submit_button.innerHTML = '<div class="spinner-border spinner-border-sm" role="status"></div>';
}
